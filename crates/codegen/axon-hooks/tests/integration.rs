//! Integration tests for axon-hooks.
//!
...
use axon_hooks::dispatcher;
use axon_hooks::event::*;
use axon_hooks::result::HookDecision;
use axon_hooks::runner::RunContext;
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$AXON_HOOK_EVENT" > {f}; echo "NAME=$AXON_HOOK_NAME" >> {f}; echo "SESSION=$AXON_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`AXON_HOOK_EVENT`, `AXON_HOOK_NAME`,
/// `AXON_SESSION_ID`, `AXON_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$AXON_HOOK_EVENT" > {f}; echo "NAME=$AXON_HOOK_NAME" >> {f}; echo "SESSION=$AXON_SESSION_ID" >> {f}; echo "ROOT=$AXON_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "AXON_HOOK_EVENT": "spoofed_event",
                                "AXON_HOOK_NAME": "spoofed_name",
                                "AXON_SESSION_ID": "spoofed_session",
                                "AXON_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "AXON_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed AXON_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "AXON_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed AXON_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "AXON_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed AXON_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
