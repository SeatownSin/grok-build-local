// Re-export all types from the standalone axon-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use axon_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `axon-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `axon-sampler` crate can reference them without a circular
// dep on `axon-shell`.

// Tests for the types now live in axon-sampling-types crate.

use axon_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `axon-sampling-types` or
/// `axon-tools`) so neither of those crates needs to depend on
