...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = axon_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                axon_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    axon_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                axon_workspace::permission::AccessKind::Read(Some(".env".into())),
                axon_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        axon_workspace::permission::Decision::PolicyDeny(_)
                    ),
