...
/// after we stop waiting. Mirrors the bound the `axon-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("AXON_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for axon_acp_lib::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
