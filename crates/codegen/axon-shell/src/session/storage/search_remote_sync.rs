...
///
/// Parsed from `[session_search.remote_sync]` in `~/.axon/config.toml`.
/// Default: disabled.
...
pub async fn maybe_upload_index(
    ...
    gcs_config: axon_file_utils::TraceExportConfig,
...
async fn upload_index_inner(
    ...
    match axon_file_utils::gcs::upload_bytes(
        &upload_config,
        &object_path,
        &compressed_bytes,
        "application/zstd",
...
impl ProxyHttpClient for crate::upload::gcs::TraceExportConfigWithAuth {
    fn proxy_http_client(&self) -> Option<reqwest::Client> {
        <Self as axon_file_utils::gcs::StorageConfig>::proxy_http_client(self)
    }
}
