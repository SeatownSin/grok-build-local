...
use axon_tools::implementations::grok_build::update_goal::{RejectReason, UpdateGoalInput};
...
fn seed_channel(actor: &SessionActor, cmds: Vec<UpdateGoalInput>) {
    ...
    *actor.goal_update_rx.borrow_mut() = Some(rx);
    for cmd in cmds {
        tx.send(axon_tools::implementations::grok_build::update_goal::envelope_for_test(cmd))
            .unwrap();
    }
    drop(tx);
}
...
async fn update_goal_tool_blocks_until_classifier_verdict_when_enabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                axon_tools::implementations::grok_build::update_goal::UpdateGoalAck::ClassifierAchieved
                { .. },),
...
async fn update_goal_tool_returns_immediately_when_classifier_disabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                axon_tools::implementations::grok_build::update_goal::UpdateGoalAck::CompletedWithoutClassifier,)
            );
        })
        .await;
}
...
async fn update_goal_tool_returns_immediately_for_blocked_reason() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                axon_tools::implementations::grok_build::update_goal::UpdateGoalAck::Accepted
                { .. },)
            );
        })
        .await;
...
async fn resolve_role_override_toolset_incapable_fails_open() {
    ...
    local
        .run_until(async {
            let mut summary =
                axon_tools::implementations::grok_build::task::types::SubagentTypeSummary {
                    can_read: true,
                    ..Default::default()
                };
