...
use axon_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
use axon_mcp::servers as inner;
...
pub fn build_config_resolved_event(
    ...
    cwd: &Path,
) -> axon_file_utils::events::Event {
    let disabled: Vec<String> = crate::util::config::disabled_mcp_server_names(cwd)
        .into_iter()
        .collect();
    let servers = configs
        .iter()
        .map(|c| axon_file_utils::events::McpConfigServer {
        ...
        .collect();
    axon_file_utils::events::Event::McpConfigResolved { servers, disabled }
}
