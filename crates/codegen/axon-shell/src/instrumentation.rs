//! Shim — see `axon_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `axon_shell::instrumentation_timer!`). Keeping the macro here
...
pub use axon_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `axon_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `axon_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `axon_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `axon_telemetry::instrumentation`.
#[macro_export]
