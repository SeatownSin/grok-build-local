//! Wire types for the `axon-workspace` API.
//!
...
//! `axon-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `axon-workspace-types` (instead of `axon-workspace` or
/// `axon-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `axon_workspace::permission`
