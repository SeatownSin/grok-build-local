...
//!
//! `axon_hooks` pulls in `git2`/`reqwest`/`axon-tools`, too heavy for
...
/// Wire mirror of `axon_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `axon_hooks::config::HookSpec`.
///
...
/// `axon-hooks`, so a server-side test (`axon-workspace`'s
...
/// Wire mirror of `axon_hooks::event::HookEventName`.
///
