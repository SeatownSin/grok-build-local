...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] axon_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `axon-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `axon_tools::ToolError`) happens
