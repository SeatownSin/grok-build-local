...
//! after every tool call to check for SKILL.md files in `.axon/skills/`,
...
/// Directories that contain skill definitions (`.axon/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".axon", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.axon/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.axon/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.axon/) are outside the git root so the walk breaks early.
