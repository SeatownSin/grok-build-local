...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.axon/skills/ or .axon/skills/)
${%- endif %}
</available_skills>"#;
...
impl axon_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> axon_tool_protocol::ToolId {
        axon_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, axon_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(axon_tool_runtime::Tool::id(&tool).as_str(), "skill");
