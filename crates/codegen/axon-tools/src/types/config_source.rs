...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.axon/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.axon/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.axon/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.axon/ or ~/.claude/.
    User { path: PathBuf },
