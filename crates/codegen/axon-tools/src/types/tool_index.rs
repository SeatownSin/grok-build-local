...
//!
//! `ToolSearchIndex` is defined in `axon-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `axon-shell`
...
/// in `axon-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `axon-shell` after MCP initialization.
#[derive(Clone)]
