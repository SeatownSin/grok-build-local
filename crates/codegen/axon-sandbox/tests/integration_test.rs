//! Integration tests for axon-sandbox.
//!
...
fn test_sandbox_logger() {
    use axon_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!axon_sandbox::should_restrict_child_network());
}
