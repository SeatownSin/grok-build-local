...
///
/// Replaces `${AXON_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${AXON_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`axon_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    axon_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".axon-plugin")).unwrap();

        // Write manifest in .axon-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".axon-plugin/plugin.json"),
    ...
    fn substitute_env_vars_replaces_all() {
        let input = "${AXON_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${AXON_PLUGIN_DATA}/cache";
