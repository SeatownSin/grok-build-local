...
//! primitive consumed cross-crate by `axon-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.axon/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.axon`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `axon-workspace`, which depends on THIS crate) to keep the dep edge
