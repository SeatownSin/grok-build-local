...
/// Directory (under `<cwd>/.axon`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.axon/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
