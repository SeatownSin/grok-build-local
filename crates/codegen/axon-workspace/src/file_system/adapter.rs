//! AcpFsAdapter: implements `axon-tools::AsyncFileSystem` using ACP gateway calls.
//!
//! This adapter enables file tool execution over ACP (remote filesystem).
//! It translates axon-tools' `AsyncFileSystem` trait into ACP protocol calls:
...
use axon_tools::computer::types::{AsyncFileSystem, ComputerError};

/// Wraps axon-shell's ACP gateway to satisfy axon-tools' AsyncFileSystem.
///
