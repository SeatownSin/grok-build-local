...
pub use axon_workspace_client::consume_stream_terminal;

/// Extract a `WorkspaceEvent` from a custom `ToolNotificationFrame`.
pub fn extract_workspace_event(
    frame: &axon_tool_protocol::ToolNotificationFrame,
) -> Option<WorkspaceEvent> {
    use axon_tool_protocol::WireToolNotification;
...
mod tests {
    use super::*;
    use axon_tool_protocol::{ToolId, ToolNotificationFrame, WireToolNotification};
    ...
    // consume_stream_terminal tests live in axon-workspace-client.
}
