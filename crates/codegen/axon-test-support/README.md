...
| `env` | `grok_binary()` (`AXON_BINARY` env → `CARGO_BIN_EXE` → local debug build of `axon-pager`), `git_workdir()` (temp git repo, forces full libgit2 init), `test_env_cmd_tokio(cmd, mock_url, home)` (sandboxed HOME **and AXON_HOME** — Windows resolves `~` via USERPROFILE, so HOME alone doesn't sandbox — + mock endpoints + telemetry kill-switches). |
...
| Consumer | Uses | Notes |
|----------|------|-------|
| `axon-shell` `tests/*.rs` | Everything | Direct imports (`use xai_grok_test_support::*` or module paths); no local shim. |
