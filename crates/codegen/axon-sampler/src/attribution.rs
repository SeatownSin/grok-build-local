...
//! it" buckets.
//!
//! `axon-sampler` is intentionally decoupled from `axon-shell`
...
/// tools live in `axon-tools`
/// (`crates/codegen/axon-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `axon_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `axon_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `axon-shell/src/auth/manager.rs` to match, otherwise the
