...
use axon_sampling_types::SamplingError;
...
/// Resolve max API retries: `AXON_MAX_RETRIES` env > model config > default ([`DEFAULT_MAX_RETRIES`]).
pub fn resolve_max_retries(model_max_retries: Option<u32>) -> u32 {
    let env_override = std::env::var("AXON_MAX_RETRIES").ok();
