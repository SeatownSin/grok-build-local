...
//!
//! Disk writes live in `axon_shell::util::config::set_<field>()`.
...
/// Default ON when unset. The `AXON_PROMPT_SUGGESTIONS` env var overrides
...
/// `AXON_SCROLL_MODE` overrides `[ui].scroll_mode` (the `AXON_SCROLL_SPEED`
...
pub fn load_scroll_mode() -> ScrollMode {
    SCROLL_MODE_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("AXON_SCROLL_MODE")
                .ok()
...
/// `AXON_INVERT_SCROLL` (`1`/`true`/`0`/`false`) overrides `[ui].invert_scroll`.
pub fn load_invert_scroll() -> bool {
    INVERT_SCROLL_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("AXON_INVERT_SCROLL")
                .ok()
...
/// its own lines-per-tick. `AXON_SCROLL_LINES` overrides `[ui].scroll_lines`.
pub fn load_scroll_lines() -> Option<u8> {
    SCROLL_LINES_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("AXON_SCROLL_LINES")
                .ok()
...
fn load_bool_from_effective_config(key: &str, default: bool) -> bool {
    let root = match axon_config::load_effective_config_disk_only() {
        Ok(r) => r,
