...
By default, each `axon -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
axon -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
axon -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
axon -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
axon -p "What's the secret number?" --resume abc123
```
...
axon -p "Write a concise commit message for these changes:
...
for file in src/*.js; do
  axon -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
RESULT=$(axon -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
axon -p "Format all files" --yolo

# Run tests and fix failures
axon -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
- Combine `--yolo` with `--rules` to set guardrails: `axon -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug axon -p "..." 2> debug.log`.
...
| `AXON_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`AXON_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
...
- Resume: `axon -p "continue" --resume "<id>"` or `axon -p "continue" --continue`
