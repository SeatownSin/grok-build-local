...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.axon/agents/` or `~/.axon/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.axon/personas/`; applied during subagent resolution |
...
Axon discovers file-based personas from these locations, in priority order:

- `.axon/personas/*.toml` (project)
- `~/.axon/personas/*.toml` (user)
...
> **Note:** Axon applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Axon resolves the effective model and reasoning effort in this order, highest priority first:
...
Axon manages worktrees through the `axon/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "local"               # route explore to a specific model
```
...
# instructions_file = ".axon/personas/concise.md"  # or load from a file
```

Axon also discovers roles from `.axon/roles/*.toml` and personas from `.axon/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Axon shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · local) — Thinking`
