...
- **`axon dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.axon/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Axon · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Axon's sibling views (
`tasks_pane`):
...
Set `AXON_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `axon --bg`") would list
