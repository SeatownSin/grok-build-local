...
//!
//! These tests exercise the real `axon-pager` binary through a PTY using
...
/// enterprise deploy report: with `AXON_GOAL=1`, `/goal` must
...
/// explicitly off (`AXON_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `AXON_FOLDER_TRUST=1` and a git repo that
...
/// also pins `AXON_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
