...
use axon_pager_pty_harness::PtyHarness;
pub(crate) use axon_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`axon-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `axon-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `AXON_SCROLL_SPEED`) appended to the controller's env.
