#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.axon` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.axon` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.axon` reads).
    #[test]
