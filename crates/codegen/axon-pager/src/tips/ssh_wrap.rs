//! SSH wrap tip: over SSH without `axon wrap`, advertise that wrapping the
...
/// Ephemeral-tip dedup key for the SSH `axon wrap` hint.
...
/// Build "Over SSH? Run `axon wrap ssh <host>` locally for clipboard +
    ...
    EphemeralTip {
        ...
        ..EphemeralTip::new(
            ...
            Line::from(vec![
                ...
                Span::styled("axon wrap ssh <host>", command),
...
mod tests {
    ...
    fn ssh_wrap_tip_advertises_local_wrap() {
        ...
        assert_eq!(
            text,
            "Over SSH? Run axon wrap ssh <host> locally for clipboard + terminal restore"
        );
    }
