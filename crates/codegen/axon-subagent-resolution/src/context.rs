...
//!
//! Extracted from `axon-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `axon-chat-state::compaction_utils::strip_system_tags` which
...
/// `axon-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use axon_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(axon_sampling_types::synthesized_reasoning_item(text))
    }
