...
//! # Wiring
//!
//! `axon-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`axon_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`axon-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `axon-pager`.
///
...
/// subsequent calls are ignored (see [`axon_pager::minimal_hook`]).
pub fn install() {
    axon_pager::minimal_hook::install(axon_pager::minimal_hook::MinimalHooks { draw });
}
